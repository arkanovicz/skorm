...
### Changed
- **Rows are a `Flow`.** `Processor.query`, `RowSetAttribute`, `Entity.browse()` and every generated accessor to many rows return `kotlinx.coroutines.flow.Flow<T>` instead of `Sequence<T>`; `Entity.iterator()` is gone (`Book.browse().collect { }`). The flow is cold, fetches rows as they are collected and closes its result set when collection ends, exhausted, abandoned or cancelled — a `Sequence` left half-consumed kept its statement out of the pool for good. `CoreProcessor` runs every connector call on a `blockingContext` (constructor argument, `Dispatchers.IO` on the JVM; the platform's IO dispatcher on native, none on JS/wasm), one hop per page of rows, so the suspend path no longer blocks the caller's thread: a Ktor handler on Netty's event loop awaits rows instead of holding the loop. `QueryResult` is `AutoCloseable`; `Processor.transaction`'s block is suspend. **Breaking**: `Sequence` call sites become `Flow` ones (`toList()`, `first()`, `map` exist for both; `forEach` becomes `collect`); consumers import `kotlinx.coroutines.flow.*`.
- **Streamed reads on JDBC.** `Connector.stream` is the many-row read (`query` stays the single-row one); `JdbcConnector` runs it on a connection of the autocommit-off pool with the statement's fetch size set (`jdbc.fetchSize`, 1000 by default), holds that connection while the flow is collected, commits and releases it when the result closes. PostgreSQL then fetches a page at a time instead of the whole result; H2 likewise; MySQL's driver buffers unless told otherwise. A write interleaved on that connection would have joined the read's transaction, which is why the connection is held rather than shared between fetches. Single-row reads keep the shared autocommit statements: a fetch by key costs no commit round trip.
...
- **`JdbcConnector(DataSource)` was unusable**: `initialize()` built the pools only when it also built the data source, so a connector given one threw at its first query.
- **Statement cache bounded**: the `maxStatements` check compared a counter nothing incremented, so the shared statements grew without limit. It is now a bound on the cache, 256 by default (the PostgreSQL driver's own per-connection default), a `StatementPool` constructor argument: past it the least recently used idle statement is closed, never one in use, and nothing throws. `getUsageStats()` counted idle statements and distinct queries; it counts statements in use and statements.
- **A transaction's statements closed**: they are prepared per use on the transaction's connection, and were never closed, only marked over; each read and write inside `transaction { }` left one open until the connection went.
