...
- **The runtime has a read-only half and a mutable half.** `Database`, `Schema`, `Entity` and `Instance` are the read-only objects: no `perform`, no mutation registered (`addAttribute` refuses one on a read-only holder), and a row's map mutators (`put`, `putAll`, `remove`, `clear`) refuse with "row is read-only" — so a row handed to a template stays read-only under reflection too. The mutable half is four interfaces the objects of a mutable database implement: `MutableDatabase` and `MutableSchema` (`perform`), `MutableEntity` (`perform` with explicit parameters, and what makes an entity's holder accept mutations), `MutableInstance` (`insert`, `update`, `upsert`, `delete`, `perform`, `putFields`, and the map mutators unlocked). For now the generator makes every generated object implement its mutable interface, so nothing changes for a consumer; the read-only classes come with the next step. Raw row loading — `putRawFields`, `putRawValue`, `putRawField`, for processors — is behind the `@SkormInternalApi` opt-in. `Instance.refresh()` no longer leaves the row dirty.
