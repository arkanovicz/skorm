...
### Changed
- **`readOnly` plugin option**: `skorm { readOnly.set(true) }` generates the read-only half alone — no `MutableExampleDatabase`, no setters, no mutations, no mutation registrations — for a build that never writes. Pinned by a `readonly` golden fixture and a compiled consumer.
- **The generator emits a read-only half and a mutable half**, and row types are interfaces. `ExampleDatabase` (read-only) and `MutableExampleDatabase : ExampleDatabase, MutableDatabase`; per schema `BookshelfSchema` and `MutableBookshelfSchema : BookshelfSchema, MutableSchema`; per table `interface Book : Instance` — `val` fields, navigations, read attributes, and a companion object that is the entity (`Book.browse()`, `Book.fetch(id)`) — and `interface MutableBook : Book, MutableInstance` — `override var` fields, the navigations redeclared to return mutable rows, the mutations (`insert()`, ksql `mut`s), its own companion entity (`MutableBook.new()`, `MutableBook.browse()`). Interfaces let a table hierarchy and mutability inherit side by side (`MutableVip : Vip, MutablePerson`), which no class shape can express; `BookImpl : InstanceImpl` and `MutableBookImpl : MutableInstanceImpl` are the storage behind them and carry the blocking twins (`@JvmName` needs a final member). **Breaking**: rows have no constructor — `Book()` becomes `MutableBook.new()`; `BookFields` is gone, `Book` is the field contract; `ExampleDatabase.bookshelf` is now an instance property (`open val`, overridden covariantly) with the companion accessor kept; an app that writes constructs `MutableExampleDatabase`, which also constructs its read-only sibling (`readOnly`, also `ExampleDatabase.instance`) over the same processor and initializes it; `initJoins()`/`initRuntimeModel()` register both halves, the mutable one only when it was instantiated. A new row of a hierarchy presets its `kind` in the mutable impl only; composites (`CurrentBorrower : DudeImpl`) are read-only rows.
- **A processor serves a store's two databases.** `CoreProcessor(connector, readConnector = connector)`: the reads of a read-only database run on the read connector — a separate pool on a SELECT-only role is the actual guarantee, `read.<tag>` in the core configuration — and the mutable database's reads run where its writes do, so they see them. Attributes are registered by path *and* mutability: both databases register the same reads at the same paths, only the mutable one registers writes, and a read-only path never resolves to a mutation. The processor initializes once, whichever database asks first; an ambient transaction is shared by the databases of one processor, so a read-only row fetched during a `transaction { }` of the mutable database comes from the transaction's connection. `Processor.eval/retrieve/query` take the caller's mutability; the REST client ignores it, the server's entities decide.
- **The runtime has a read-only half and a mutable half.** `Instance` is now an interface — a read-only map of the row's fields, with the kson getters — so that a generated row type can be an interface too and inherit along two axes, the table hierarchy and mutability; `InstanceImpl` (`Json.Object` owning its storage) and `MutableInstanceImpl` (`Json.MutableObject`, dirty tracking, the typed `put`) are the storage behind them. A read-only row has no `put`, `remove` or `clear`: nothing to refuse. `Database`, `Schema`, `Entity` are the read-only objects: no `perform`, no mutation registered (`addAttribute` refuses one on a read-only holder). The mutable half is four interfaces the objects of a mutable database implement: `MutableDatabase` and `MutableSchema` (`perform`), `MutableEntity` (`perform` with explicit parameters, and what makes an entity's holder accept mutations), `MutableInstance` (`put`, `putFields`, `insert`, `update`, `upsert`, `delete`, `perform`). Rows read into by an attribute are `Row`s (`Map<String, Any?>`): an `Instance` or a plain kson object, where the bounds used to say `Json.MutableObject`. For now the generator makes every generated object mutable, so nothing changes for a consumer; the read-only types come with the next step. Raw row loading — `putRawFields`, `putRawValue`, `putRawField`, for processors — is behind the `@SkormInternalApi` opt-in. `Instance.refresh()` no longer leaves the row dirty.
- **Every read accessor has a blocking twin**, for callers that cannot suspend — a template engine resolving `$book.tags()` by reflection. Beside `suspend fun tags(): Sequence<Tag>` the class declares `@JvmName("tags") fun tagsBlocking(): List<Tag>`: a reflection-driven caller finds `tags()` with no parameters and gets the twin, Kotlin code writes `tags()` or `tagsBlocking()`, no clash between them. Row sets come back as a `List`, a `Sequence` being single-pass. Mutations have no twin. The twin dispatches on `Database.blockingContext` (by default the calling thread) and, when called while a `transaction { }` runs, joins that transaction — a render inside it sees its own writes, on the same connection. On JS and wasm the twin exists and throws `UnsupportedOperationException`; native blocks like the JVM but joins no transaction. Runtime: `blockingOn` (expect/actual), `Database.blocking`, and a thread companion installed by `transaction { }`.
- **Navigations and ksql attributes are members, not extension functions.** `book.author()`, `book.tags()`, `book.currentBorrower()`, `book.lend(…)` are declared inside the generated `Book`; a schema-level attribute (`booksCount()`, `newBookBy(…)`) inside its schema class. Same call sites compile as before, without the import a Kotlin extension needed; reflection-driven tools (a template engine) now see them; a subtype inherits its parent's. `skormJoins.kt` is no longer generated, and `skormModel.kt` keeps only the composite row classes. Only an explicit `import …model.<accessor>` breaks.
