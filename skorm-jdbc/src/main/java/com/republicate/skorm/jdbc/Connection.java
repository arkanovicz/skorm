    ...
    implements java.sql.Connection, Serializable
{
    ...
    /** Infos on the driver. */
    private transient Vendor vendor = null;
...
-     * @param connection connection to be wrapped
    ...
    public synchronized Vendor getVendor() throws SQLException
    {
        if (vendor == null)
        {
            vendor = Vendor.getVendor(getMetaData());
        }
        return vendor;
    }
    ...
    {
        long ret = -1;
        switch (vendor.getLastInsertIdPolicy())
        {
            case METHOD:
            {
                ...
                if (lastInsertIdMethod == null)
                {
                    synchronized(this)
                    {
                        if (lastInsertIdMethod == null)
                        {
                            try
                            {
                                String classAndMethod = vendor.getLastInsertIdMethod();
            ...
            case QUERY:
            {
                ResultSet rs = statement.getConnection().createStatement().executeQuery(vendor.getLastInsertIdQuery());
                rs.next();
                ret = rs.getLong(1);
                if (rs.wasNull())
                {
    ...
    public synchronized boolean check()
    {
        try
        {
            String checkQuery = vendor.getPingQuery();
            if (checkQuery == null)
            {
